...
#define ESP_UART_NODE DT_NODELABEL(uart1)

static const struct device *esp_uart = DEVICE_DT_GET(ESP_UART_NODE);

static void esp_uart_send(const char *text)
{
	if (!device_is_ready(esp_uart)) {
		printf("ESP UART not ready\n");
		return;
	}

	for (size_t i = 0; i < strlen(text); i++) {
		uart_poll_out(esp_uart, text[i]);
	}
}

static void send_location_to_esp(struct nrf_modem_gnss_pvt_data_frame *pvt_data)
{
	char msg[160];

	if (pvt_data->flags & NRF_MODEM_GNSS_PVT_FLAG_FIX_VALID) {
		snprintk(msg, sizeof(msg),
			 "LOC,%.06f,%.06f,%.01f,%04u-%02u-%02uT%02u:%02u:%02u.%03uZ\n",
			 pvt_data->latitude,
			 pvt_data->longitude,
